...
begin
  ...
  fmAddPostavshik := TfmAddPostavshik.Create(Application);
...
//Создать форму fmAddNakladnaya
  fmAddNakladnaya := TfmAddNakladnaya.Create(Application);
  //Выполняем на этой форме добавление записи
...
begin
  ...
  else fmEditPostavshik := TfmEditPostavshik.Create(Application);
...
//Проверить существуют ли записи в таблице
  ...
  //Создать форму fmEditNakladnaya
  else fmEditNakladnaya := TfmEditNakladnaya.Create(Application);
   //Выполняем на этой форме изменение записи
